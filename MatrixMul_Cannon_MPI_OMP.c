///////////////////////////////////////////////////////////////////////////////////////////
// MatrixMul_Cannon_MPI_OMP.c
// --------------------------------------------------------------------------------------
// Cannon's Algorithm for Matrix Multiplication using Hybrid MPI + OpenMP
// 
// Design:
// 1. Assumes perfect square matrices and q×q process grid (p = q²)
// 2. Root reads matrices and distributes tiles to all processes
// 3. Initial skew: Shift A tiles left by row index, B tiles up by column index
// 4. Each process performs q iterations of Cannon's algorithm:
//    - Multiply local tiles using OpenMP
//    - Shift A tiles left (circular)
//    - Shift B tiles up (circular)
// 5. Root gathers and writes final result
///////////////////////////////////////////////////////////////////////////////////////////
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        printf("\n=== Cannon's Algorithm: Hybrid MPI + OpenMP Matrix Multiplication ===\n");
    ...
    if (rank == 0) {
        ...
        if (rowA != colA || rowB != colB || rowA != rowB) {
            printf("Error: Matrices must be square and same size for Cannon's algorithm\n");
    ...
    // ==================== INITIAL SKEW (CANNON'S ALIGNMENT) ====================
    if (rank == 0) {
        printf("\n[All Processes] Performing initial skew - Start\n");
    }
    ...
    // Initial skew for A: Shift left by row index
    if (my_row > 0) {
        int *temp_A = (int*)malloc(tile_size * tile_size * sizeof(int));
        int src = (my_col + my_row) % q;
        int dest = (my_col - my_row + q) % q;
        
        MPI_Sendrecv(tile_A, tile_size * tile_size, MPI_INT, dest, 10,
                     temp_A, tile_size * tile_size, MPI_INT, src, 10,
                     row_comm, MPI_STATUS_IGNORE);
        
        memcpy(tile_A, temp_A, tile_size * tile_size * sizeof(int));
        free(temp_A);
    }
    
    // Initial skew for B: Shift up by column index
    if (my_col > 0) {
        int *temp_B = (int*)malloc(tile_size * tile_size * sizeof(int));
        int src = (my_row + my_col) % q;
        int dest = (my_row - my_col + q) % q;
        
        MPI_Sendrecv(tile_B, tile_size * tile_size, MPI_INT, dest, 11,
                     temp_B, tile_size * tile_size, MPI_INT, src, 11,
    ...
    if (rank == 0) {
        printf("[All Processes] Initial skew - Done\n");
    }
    
    // ==================== CANNON'S ALGORITHM ====================
    if (rank == 0) {
        printf("\n[All Processes] Cannon's Algorithm - Start\n");
    }
    
    // Cannon's algorithm: q iterations
    for (int stage = 0; stage < q; stage++) {
        // Step 1: Local multiplication using OpenMP
        multiply_tiles(tile_A, tile_B, tile_C, tile_size);
        
        // Step 2: Shift A tiles left (circular)
        int *temp_A = (int*)malloc(tile_size * tile_size * sizeof(int));
        int left = (my_col - 1 + q) % q;
        int right = (my_col + 1) % q;
        
        MPI_Sendrecv(tile_A, tile_size * tile_size, MPI_INT, left, 20,
                     temp_A, tile_size * tile_size, MPI_INT, right, 20,
                     row_comm, MPI_STATUS_IGNORE);
        
        memcpy(tile_A, temp_A, tile_size * tile_size * sizeof(int));
        free(temp_A);
        
        // Step 3: Shift B tiles upward (circular)
        int *temp_B = (int*)malloc(tile_size * tile_size * sizeof(int));
        int up = (my_row - 1 + q) % q;
        int down = (my_row + 1) % q;
        
        MPI_Sendrecv(tile_B, tile_size * tile_size, MPI_INT, up, 21,
                     temp_B, tile_size * tile_size, MPI_INT, down, 21,
                     col_comm, MPI_STATUS_IGNORE);
    ...
    if (rank == 0) {
        printf("[All Processes] Cannon's Algorithm - Done\n");
    }
    ...
    if (rank == 0) {
        ...
        printf("\nCannon's Algorithm Matrix Multiplication - Done\n\n");
